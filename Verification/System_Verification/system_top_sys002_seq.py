...
class sys002_chirp_loopback_Seq(uvm_sequence):
    """
    SYS-002: Single-Frame Loopback — Chirp.
    Applies a sweeping LFM chirp (f0 = 0 MHz, B = 200 MHz). The OFDM
    memory is left uninitialized so only the radar path is exercised.
    RX is expected to recover the chirp through FFT -> conjugate
    multiplier -> IFFT, producing a range-profile peak at zero delay.
    """
    def __init__(self, name="sys002_chirp_loopback_seq"):
        super().__init__(name)
        self.num_frames = 5
    ...
    async def body(self):
        ...
        for i in range(self.num_frames):
            ...
            # ==========================================
            # OFDM memory intentionally left uninitialized - chirp-only test
            ...
            # ==========================================
            # Hardware runs the pipeline: TX chirp -> channel -> RX range profile
