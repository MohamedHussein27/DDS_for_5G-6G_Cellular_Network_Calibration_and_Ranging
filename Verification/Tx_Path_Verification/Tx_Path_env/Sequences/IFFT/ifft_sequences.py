...
class impulse_sequence(uvm_sequence):
    ...
    async def body(self):
        ...
        X[0] = 1.0 + 0j
        #X[4095] = 1.0 + 0j # to check the symmetry of the IFFT output
        pairs = _frame(X)
        ...
        for re, im in pairs:
            ...
            # Only use the solver for control signals
            self.seq_item.randomize_with(
                lambda rst_n, valid_in: rst_n == 1 and valid_in == 1
            )
            
            # Directly assign deterministic data to bypass solver overhead
