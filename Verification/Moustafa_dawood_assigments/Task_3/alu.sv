...
module ALU(
    ...
    // 1. Synchronous Output Update (Registered Output)
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            {c, out} <= 5'b0;
        end
    ...
    // 2. Combinational Logic (The Calculation)
    always @(*) begin
        case (op)
            2'b00: out_data = a + b;       // ADD
            2'b01: out_data = a ^ b;       // XOR
            2'b10: out_data = a & b;       // AND
            2'b11: out_data = a | b;       // OR
            default: out_data = 5'b0;
        endcase
    end
