module real_comparator #(
    ...
    input wire signed [INPUT_WIDTH-1:0] OFDM_real_in,

    input wire signed [INPUT_WIDTH-1:0] OFDM_real_ref_in,

    input wire signed [INPUT_WIDTH-1:0] radar_real_in,

    input wire signed [INPUT_WIDTH-1:0] radar_real_ref_in,
    ...
    reg signed [INPUT_WIDTH-1:0] ofdm_real_in_reg;
    ...
    always @(posedge clk or negedge rst_n) begin
        ...
        if (!rst_n) begin
            ...
            // Register input
             ofdm_real_in_reg <= OFDM_real_in;

            // Store comparison result
            // symbol_array_ofdm[symbol_index_ofdm] <= 
            //     (ofdm_real_in_reg[INPUT_WIDTH-1:6] == OFDM_real_ref_in[INPUT_WIDTH-1:6]);

            if(ofdm_real_in_reg[INPUT_WIDTH-1:6] == OFDM_real_ref_in[INPUT_WIDTH-1:6]) begin
    ...
    reg signed [INPUT_WIDTH-1:0] radar_real_in_reg;
    ...
    always @(posedge clk or negedge rst_n) begin
        ...
        if (!rst_n) begin
            ...
            // Register input
             radar_real_in_reg <= radar_real_in;

            // Store comparison result
            // symbol_array_radar[symbol_index_radar] <= 
            //     (radar_real_in_reg[INPUT_WIDTH-1:6] == radar_real_ref_in[INPUT_WIDTH-1:6]);
            
            if(radar_real_in_reg[INPUT_WIDTH-1:6] == radar_real_ref_in[INPUT_WIDTH-1:6]) begin
